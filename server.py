...
sockets_list = [server_socket] ## creates a list of all connections to server
...
def receive_message(client_socket):
    ...
    try:
        ...
        message_header = client_socket.recv(HEADER_LENGTH) ### this is ENCODED length of message
        ...
        message_length = int(message_header.decode('utf-8').strip()) ## ### this is DECODED length of message

        # Return an object of message header and message data, which means returning the ENCODED message length and ENCODED actual message(retrived with actual message length)
...
while True: ## this loops over the list of SOCKET CONNECTIONS, if the connection is server socket then adds it to the list of sockets else retrieves the message from the socket
    ...
    for notified_socket in read_sockets:
        ...
        if notified_socket == server_socket:
            ...
            # Also save username and username header, creating a dictionary of user name length and username
            clients[client_socket] = user

            print('Accepted new connection from {}:{}, username: {}'.format(*client_address, user['data'].decode('utf-8'))) ## user value is DECODED
